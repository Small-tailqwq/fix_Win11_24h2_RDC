...
if (Test-Path (Join-Path $Sys32LangPathCandidate "mstsc.exe.mui")) {
    ...
    # 使用双语输出提示
    $params = @{
         MessageZh = "使用 .NET 获取的系统语言 ($InstalledLang) 在 $System32Path 下未找到 mstsc.exe.mui 文件，尝试采用备选列表查找…"
         MessageEn = "The system language detected via .NET ($InstalledLang) did not yield mstsc.exe.mui in $System32Path; trying backup language list search..."
         Color     = "Yellow"
    }
    ...
    if (-not $DetectedLanguage) {
        $params = @{
             MessageZh = "未能找到 mstsc.exe.mui 文件，请检查系统是否支持远程桌面或相关文件是否存在。"
             MessageEn = "Could not locate mstsc.exe.mui. Please check if the system supports Remote Desktop or if the corresponding files exist."
             Color     = "Red"
        }
        Write-Message @params
        Pause
